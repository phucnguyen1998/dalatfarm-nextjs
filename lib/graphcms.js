...
export async function getAllCategoriesName() {
  const data = await fetchAPI(`
    {
      productCatogoriess(orderBy: id_ASC) {
        slug
        categoryName
      }
    }
  `)
  return data.productCatogoriess
}

export async function getAllBrand() {
  const data = await fetchAPI(`
    {
      partners(orderBy: id_ASC) {
        companyName
        logo {
          url
        }
...
export async function getProductByCategories(slug) {
  const data = await fetchAPI(
    `
    {
      productCatogories(where: {slug: "${slug}"}) {
        categoryName
        products(first: 100) {
          product_content {
               markdown
